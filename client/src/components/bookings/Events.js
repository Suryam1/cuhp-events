import React, { useEffect, useState } from "react";
import axios from "axios";
import LoadingSpinner from "../LoadingSpinner";
import EventCard from "./EventCard";
import { isBefore } from "date-fns/esm";
import { format, subDays } from "date-fns";

const Events = () => {
  const [eventData, setEventData] = useState({});
  const [isLoading, setIsLoading] = useState(true);

  const getEventData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_SERVER_URL}/events`,
        {
          // withCredentials: true, // include credentials in the request
          headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
          },
        }
      );

      const data = response.data.bookings;
      //consolelog(data);

      const sortedEventData = data.sort((a, b) => {
        // Convert the event date strings to Date objects and compare them
        return new Date(a.eventDate) - new Date(b.eventDate);
      });
      const firstDateIsPastDayComparedToSecond = (firstDate, secondDate) => {
        if (isBefore(secondDate, firstDate)) {
          return true;
        }

        return false;
      };
      const eventstoday = sortedEventData.filter((e) => {
        //console.log(format(new Date(), "yyyy-MM-dd'T'HH:mm:ssXX"));
        // console.log(
        //   new Date(format(subDays(new Date(), 1), "yyyy-MM-dd'T'HH:mm:ssXX"))
        // );
        let a;
        if (e.eventDate === null) {
          a = e.eventEndDate;
        } else {
          a = e.eventDate;
        }
        return firstDateIsPastDayComparedToSecond(
          new Date(a),
          new Date(format(subDays(new Date(), 1), "yyyy-MM-dd'T'HH:mm:ssXX"))
        );
      });
      setEventData(eventstoday);

      setIsLoading(false);
      if (response.status !== 200) {
        throw new Error(response.status);
      }
    } catch (error) {
      console.log(error);
    }
  };

  useEffect(() => {
    getEventData();
  }, []);

  return (
    <>
      <div className="mt-6 min-h-screen">
        <h1 className="text-xl sm:text-3xl md:text-4xl lg:text-3xl xl:text-3xl text-center text-gray-800 font-black leading-7 ml-3 md:leading-10">
          Upcomming<span className="text-indigo-700"> Events</span>{" "}
        </h1>
        {isLoading ? (
          <LoadingSpinner />
        ) : Array.isArray(eventData) && eventData.length ? (
          eventData.map((eventsData) => <EventCard eventsData={eventsData} />)
        ) : (
          <h2 className="text-2xl font-bold text-zinc-700  text-center mt-10">
            No Upcomming Events.
          </h2>
        )}
      </div>
    </>
  );
};

export default Events;
